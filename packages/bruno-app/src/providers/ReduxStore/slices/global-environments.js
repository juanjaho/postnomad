...
export const globalEnvironmentsSlice = createSlice({
  ...
  reducers: {
    ...
    _updateGlobalEnvironmentColor: (state, action) => {
      ...
      if (environmentUid) {
        state.globalEnvironments = state.globalEnvironments.map((env) =>
          env?.uid == environmentUid ? { ...env, color } : env
        );
      }
    }
  }
  ...
  (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      ...
      environmentSchema
        ...
        .then(() =>
          ipcRenderer.invoke('renderer:create-global-environment', {
            name,
            uid,
            variables,
            color,
        ...
        .then((result) => {
          ...
          dispatch(
            _addGlobalEnvironment({ name: finalName, uid: finalUid, variables: finalVariables, color: finalColor })
          );
          return finalUid;
        })
...
export const copyGlobalEnvironment =
  ({ name, environmentUid: baseEnvUid }) =>
  (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      const state = getState();
      const { workspaceUid, workspacePath } = getWorkspaceContext(state);
  ...
  (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      ...
      environmentSchema
        .validate(environment)
        .then(() =>
          ipcRenderer.invoke('renderer:rename-global-environment', {
            name: newName,
            environmentUid,
            workspaceUid,
            workspacePath
  ...
  (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      ...
      environmentSchema
        ...
        .then(() =>
          ipcRenderer.invoke('renderer:save-global-environment', {
            environmentUid,
            variables,
...
export const updateGlobalEnvironmentColor = (environmentUid, color) => (dispatch, getState) => {
  return new Promise((resolve, reject) => {
    ...
    const { workspaceUid, workspacePath } = getWorkspaceContext(state);
    ipcRenderer
      .invoke('renderer:update-global-environment-color', { environmentUid, color, workspaceUid, workspacePath })

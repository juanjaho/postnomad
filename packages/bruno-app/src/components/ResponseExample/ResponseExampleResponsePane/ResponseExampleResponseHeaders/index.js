...
const ResponseExampleResponseHeaders = ({ editMode, item, collection, exampleUid }) => {
  ...
  const headers = useMemo(() => {
    return item.draft
      ? get(item, 'draft.examples', []).find((e) => e.uid === exampleUid)?.response?.headers || []
      : get(item, 'examples', []).find((e) => e.uid === exampleUid)?.response?.headers || [];
  }, [item, exampleUid]);
  ...
  if (isBulkEditMode && editMode) {
    ...
    return (
      <StyledWrapper className="w-full overflow-auto">
        <BulkEditor params={headersForBulkEdit} onChange={handleBulkHeadersChange} onToggle={toggleBulkEditMode} />
      </StyledWrapper>
    );
  }

...
import { IconCheck, IconRefresh, IconAlertTriangle, IconClock } from '@tabler/icons';
...
const SpecStatusSection = ({
  ...
  const hasRemoteUpdates =
    remoteDrift &&
    (remoteDrift.missing?.length || 0) + (remoteDrift.modified?.length || 0) + (remoteDrift.localOnly?.length || 0) > 0;
  ...
  const bannerState = useMemo(() => {
    ...
    if (error || specDrift?.isValid === false) {
      return {
        variant: 'danger',
        message: error || specDrift?.error || 'Invalid OpenAPI specification',
    ...
    if (hasEndpointUpdates) {
      ...
      return {
        variant: 'warning',
        message: `OpenAPI spec has been updated${versionInfo}`,
        actions: [],
        changes: {
    ...
    <>
              ...
              <span className="banner-title">
                ...
                {bannerState.version && (
                  <>
                    {' '}
                    &middot;{' '}
                    <code style={{ fontStyle: 'normal' }} className="checked-text">
                ...
                <span className="banner-details">
                  {bannerState.changes.modified > 0 && (
                    <StatusBadge key="modified" status="warning" radius="full">
                      {bannerState.changes.modified} {bannerState.changes.modified > 1 ? 'endpoints' : 'endpoint'}{' '}
                      updated
                    </StatusBadge>
                  )}
                  {bannerState.changes.added > 0 && (
                    <StatusBadge key="added" status="success" radius="full">
                      {bannerState.changes.added} {bannerState.changes.added > 1 ? 'endpoints' : 'endpoint'} added
                    </StatusBadge>
                  )}
                  {bannerState.changes.removed > 0 && (
                    <StatusBadge key="removed" status="danger" radius="full">
                      {bannerState.changes.removed} {bannerState.changes.removed > 1 ? 'endpoints' : 'endpoint'} removed
                    </StatusBadge>
                  )}
                </span>
              )}
      ...
      {error || fileNotFound || specDrift?.isValid === false ? (
        ...
        <div className="sync-review-empty-state mt-5">
          ...
          <p>
            The spec endpoints have not been updated since the last sync. You can restore the spec file to track local
            collection changes.
          </p>

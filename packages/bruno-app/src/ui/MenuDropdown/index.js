...
const MenuDropdown = forwardRef(
  (
    ...
    const getMenuItems = useCallback(() => {
      ...
      return Array.from(menuContainer.querySelectorAll('[role="menuitem"]:not([aria-disabled="true"])'));
    }, []);
    ...
    const getMenuItemProps = (item, extraProps = {}) => {
      ...
      return {
        className:
          `dropdown-item ${item.disabled ? 'disabled' : ''} ${selectIndentClass} ${activeClass} ${extraClassName || ''} ${item.className || ''}`.trim(),
        role: 'menuitem',
    ...
    const renderMenuItem = (item) => {
      ...
      return (
        <div key={item.id} {...itemProps} onClick={() => !item.disabled && handleItemClick(item)}>
